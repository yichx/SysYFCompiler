...
$$
In[B]=UperExposed[B]+PhiUse[B]+(Out[B]-Defs[B])\\
Out[B]=\cup_{S是B的后继} UperExposed[S]+(Out[S]-Defs[S])\cup PhiFromSuc[B]
$$

UperExposed:表示向上暴露的变量（被引用而且没有被定值）。

PhiUse 在Phi中引用的变量。

Defs 基本块中定值的变量。

PhiFromSuc：后继基本块引用当前基本块的变量。

上述所有集合，除了In和Out都可以直接从当前基本块中得到，在迭代过程中是不变量。所以先计算出上述所有集合，然后迭代计算数据流方程即可。

### 检查器

#### B4-1

进行的检查：不同基本块之间的前驱后继关系，指令的操作数必须有定值才能使用，基本块必须以br、ret等指令结尾，def-use链等

不同基本块之间的前驱后继关系：检查前驱是否和后继是否一一对应。

指令的操作数必须有定值：先找出当前函数的所有定值，然后对于每个操作数查看是否在定值集合里面。

基本块必须以br、ret等指令结尾：检查当前基本块最后一个指令的类型

def-use链：检查某个函数的use_list对应指令的对应位置是否是当前指令

插入的位置：在execute之前插入`passmgr.addPass<Check>`，在做完优化之后检查
